...
const (
	// LevelTrace enables extremely verbose logs (protocol events, FSM inputs,
	// etc).
...
func ParseLevel(raw string) (Level, error) {
	...
	default:
		return LevelInfo, fmt.Errorf(
			"invalid log level %q (expected trace, debug, info, warn, or error)",
			raw,
		)
	}
}

// state holds the global logging configuration.
type state struct {
	mu     sync.RWMutex
	level  Level
	logger *log.Logger
...
// global is the singleton logger used throughout Delight.
var global = state{
	level:  LevelInfo,
...
// Enabled reports whether a level would be emitted by the current
// configuration.
...
func logf(level Level, format string, args ...any) {
	...
	// Avoid fmt.Sprintf so formatting is skipped when level is disabled.
